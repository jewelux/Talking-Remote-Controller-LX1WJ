/*
  ...
  File: TalkingRemoteControllerLX1WJ_V1.A.ino
  Version: V1.A  (stable core; ongoing development)
...
// Optional extra voice tokens present in newer voice_data.h
// (define the HAS_VOICE_* flags so the code can use them)
#define HAS_VOICE_voice_xiegu 1
#define HAS_VOICE_voice_r 1

// ============================================================
// USER CONFIG (before compiling)
// ============================================================
...
static constexpr CivProfile PROFILE_706_CIV  = { 0x58, "Icom 706", CIV_BAUD, 1, CIV_RX_PIN, CIV_TX_PIN, true, false };
// Profile 3: IC-706 command set over RS232 (CT-17 / MAX3232) for further testing
static constexpr CivProfile PROFILE_706_RS232 = { 0x58, "Icom 706 rs232", CIV_BAUD, 2, RS232_RX_PIN, RS232_TX_PIN, false, false };
...
static constexpr CivProfile PROFILE_G106_CAT = { 0x76, "Xiegu 106", CAT_BAUD, 2, CAT_RX_PIN, CAT_TX_PIN, CAT_TX_INVERT, CAT_RX_INVERT };
...
static void speakBootProfile() {
  ...
  if (g_profileId == PROFILE_ID_7300) {
    ...
    if (g_profileId == PROFILE_ID_706_RS232) {
      // Speak "rs232" suffix to avoid confusion with the CI-V profile.
#ifdef HAS_VOICE_voice_r
      playSilenceMs(60);
      playClipProgmem(voice_r, voice_r_len);
      playSilenceMs(40);
#endif
#ifdef HAS_VOICE_voice_s
      playClipProgmem(voice_s, voice_s_len);
      playSilenceMs(40);
#endif
      playDigitsFromCString("232");
    }
  } else {
    playDigitsFromCString("7300");
  }
...
// Say: "icom" + ("seven three zero zero" OR "seven zero six") + "ok"
  ...
  if (g_profileId == PROFILE_ID_7300) {
    ...
    // Both profile 2 and 3 are IC-706 (different physical link). Speak "706", and add "r" for RS232.
#ifdef HAS_VOICE_voice_seven
    playClipProgmem(voice_seven, voice_seven_len); playSilenceMs(60);
#endif
#ifdef HAS_VOICE_voice_zero
...
// Bank 1: HOLD '9' -> enter MODE SET flow (digit 1..9 sets mode)
  ...
  if (g_bank == 3 && k == 'A') {
    if (s == HOLD) {
      ...
      Serial.println("[KP] PROFILE SELECT: press 1=IC-7300, 2=Icom 706 (CI-V), 3=Icom 706 rs232, 4=Xiegu 106");
      if (g_speechEnabled) {
